...
def demo():
    ...
    products = pd.DataFrame({
        'id': [1, 2, 3, 4, 5, 6, 7],
        'name': ['Golden Delicious', 'Yellow Delicious', 'Red Delicious', 'Strawberry', 'Cherry', 'Blue Strawberry', 'Black Cherry'],
    ...
    # Basic semantic DISTINCT with LOTUS
    lotus_result = engine.query_ast("""
    SELECT SEM_DISTINCT(name, 'lotus') AS feature
    FROM products
    """)
    print("SEM_DISTINCT for products based on name:")
    print(lotus_result.head(10))
    print()

    # Basic semantic DISTINCT with DocETL
    docetl_result = engine.query_ast("""
    SELECT SEM_DISTINCT(name, 'docetl') AS feature
    FROM products
    """)
    print("SEM_DISTINCT for products based on name:")
